...
## Implemented Requirements (T02 — crm-tech-debt-cleanup, 2026-08-03)

The following 6 permissions were added to `ALL_PERMISSIONS` in `src/shared/auth/rbac.js` by T02 of `crm-tech-debt-cleanup`. The scenarios above were already documented in this spec; the permissions were implemented to satisfy them.
...
- `auditoria.read` — required by `AdminAuditPanel`
- `backup.admin` — required by `BackupPanel`
- `usuarios.write` — required by `UsuariosList`
- `leads.write` — required by `LeadsPanel` / `ClienteDrawer`
- `gbp.write` — required by `GbpPanel`
- `agenda.snapshots` — required by `AgendaGlobalPanel` `gbp_snapshot` toggle

**Implementation note**: All 6 permissions were verified present in `ALL_PERMISSIONS` and all 8 component gates were migrated to use the correct granular permissions (verify-report: crm-tech-debt-cleanup, 2026-08-03).
