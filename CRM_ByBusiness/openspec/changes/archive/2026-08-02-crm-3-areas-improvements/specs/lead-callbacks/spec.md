...
The system MUST ensure that actions in `MisCallbacksPanel` do not conflict with `CRM_WATCHDOG_CALLBACKS` processing.

#### Scenario: Callback cancelled before watchdog processes it

- GIVEN an operator cancels a callback via `MisCallbacksPanel`
- WHEN `CRM_WATCHDOG_CALLBACKS` runs within the same time window
- THEN the watchdog detects the callback is already cancelled and skips it
