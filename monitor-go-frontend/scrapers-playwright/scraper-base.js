    ...
    async extractListData(page) {
        return await page.evaluate(() => {
            ...
            cards.forEach(card => {
                ...
                const ratingSpan = container.querySelector('span[aria-label*="estrellas"], span[aria-label*="stars"]');
                let rating = null;
                let reviews = null;
    ...
    async extractBasicData(page) {
        try {
            ...
            const address = await page.evaluate(() => {
                ...
                const selectors = ['.Io6YTe.fontBodyMedium.kR99db', '.LbwUeb.fontBodyMedium', '[data-item-id="address"]'];
            ...
            const rating = await page.evaluate(() => {
                 ...
                 const span = document.querySelector('span[aria-label*="estrellas"], span[aria-label*="stars"]');
                 if (span) {
                 ...
                 const ratingText = document.querySelector('.ce4Yce span[role="img"]')?.getAttribute('aria-label');
                 if (ratingText) {
