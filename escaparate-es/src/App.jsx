    ...
    <nav className="lg:hidden flex-none h-16 bg-white border-t border-slate-100 flex relative z-[200]">
      {tabs.map(({ label, icon: Icon, action, active }) => (
        <button
          key={label}
          onClick={action}
          className={`flex-1 flex flex-col items-center justify-center gap-0.5 relative transition-colors active:bg-slate-50 ${active ? 'text-blue-600' : 'text-slate-400'
            }`}
        >
...
function App() {
  ...
  return (
    <div className="h-screen overflow-hidden w-screen flex flex-col bg-white font-sans select-none relative">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest leading-relaxed">
                Uso de cookies técnicas según normativa RGPD España/UE.
              </p>
            </div>
            <button
      ...
      {/* Widgets WhatsApp — subidos en mobile para no solapar bottom nav */}
      <WhatsAppWidget />
      <WhatsAppAssistant />
    </div>
  );
